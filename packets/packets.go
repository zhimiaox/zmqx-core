...
const (
	...
	// MaxSize The maximum packet size of a MQTT packet
	MaxSize = 268435456
)
...
func NewReader(r io.Reader) *Reader {
	if bufReader, ok := r.(*bufio.Reader); ok {
		return &Reader{bufReader: bufReader, version: Version311}
	}
	return &Reader{bufReader: bufio.NewReaderSize(r, 2048), version: Version311}
}
...
func NewWriter(w io.Writer) *Writer {
	if bufWriter, ok := w.(*bufio.Writer); ok {
		return &Writer{bufWriter: bufWriter}
	}
	return &Writer{bufWriter: bufio.NewWriterSize(w, 2048)}
}

// ReadPacket reads data from Reader and returns a  Packet instance.
// If any errors occurs, returns nil, error
func (r *Reader) ReadPacket() (Packet, error) {
	first, err := r.bufReader.ReadByte()
	if err != nil {
		return nil, err
	}
	fh := &FixHeader{PacketType: first >> 4, Flags: first & 15} // 设置FixHeader
	length, err := EncodeRemainLength(r.bufReader)
	if err != nil {
		return nil, err
	}
	fh.RemainLength = length
	packet, err := NewPacket(fh, r.version, r.bufReader)
	if err != nil {
		return nil, err
	}
...
func EncodeUTF8String(buf []byte) ([]byte, int, error) {
	bufLen := len(buf)
	if bufLen > 65535 {
		return nil, 0, errors.ErrMalformed
	}
	// length := int(binary.BigEndian.Uint16(buf[0:2]))
	ret := make([]byte, 2, 2+bufLen)
	binary.BigEndian.PutUint16(ret, uint16(bufLen))
	ret = append(ret, buf...)
	return ret, 2 + bufLen, nil
}
...
func ValidV5Topic(p []byte) bool {
	...
	if bytes.HasPrefix(p, []byte("$share/")) {
		...
		if p[7] != '/' {
			tail := p[7:]
			for len(tail) > 0 {
				ru, size := utf8.DecodeRune(tail)
				if ru == utf8.RuneError {
					return false
				}
				if size == 1 {
					if tail[0] == '/' {
						return ValidTopicFilter(true, tail[1:])
					}
					if tail[0] == byte('+') || tail[0] == byte('#') {
						return false
					}
				}
				tail = tail[size:]
...
func ValidTopicFilter(mustUTF8 bool, p []byte) bool {
	...
	for len(p) > 0 {
		...
		pLen := len(p)
		// # must be the last character tail
		if p[0] == byte('#') && pLen != 1 {
			return false
		}
		if size == 1 && isSetPrevByte {
			// the characters before and after + must be "/" [MQTT-4.7.1-2]  [MQTT-4.7.1-3]
			if (p[0] == byte('+') || p[0] == byte('#')) && prevByte != byte('/') {
				return false
			}

			// if p[0] is not the last byte
			if pLen > 1 {
				// the characters before and after + must be "/" [MQTT-4.7.1-2]  [MQTT-4.7.1-3]
...
func TopicMatch(topic []byte, topicFilter []byte) bool {
	var spos, tpos int
	topiclen, sublen := len(topic), len(topicFilter)
